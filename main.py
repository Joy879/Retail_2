...
data = st.sidebar.file_uploader("Upload file")
if data is not None:
        try:
  ...
  #         df = pd.read_csv(url, encoding="ISO-8859-1", low_memory=False)
          ...
          df["InvoiceYear"] = pd.DatetimeIndex(df["InvoiceDate"]).year
             ...
             st.sidebar.error("Please submit the right file. find an example shown below")
            #  st.image()
...
# st.sidebar.text("Please submit data of a certain kind")
...
if selection== 'About':
    ...
    st.write("This is a retail dashboard designed by the developers")
